...
| [x] | Surface a document's folder path | A document tab's hover tooltip shows `Folder/Sub/Title`, derived live from the sidebar tree (never stored on the cookie-persisted tab), and the same path renders as a static breadcrumb above the title in the editor and read-only views |
| [x] | Consolidate folder actions into a Folder settings dialog | Default tags and Sharing are tabs of one dialog (`components/folder-settings-dialog.tsx`); the folder three-dot menu no longer forces the action labels to wrap two lines each — `components/ui/dropdown-menu.tsx` was sizing every menu to its 24px trigger via `w-(--anchor-width)` |
| [~] | Final workspace visual pass | Document editor route now uses a centered Obsidian-like editor column with Read/Live/Source modes, a compact hover/focus icon stack for mode switching, and live-mode typography/list/callout styling closer to Read mode; new tab is a raw greeting/search/recent surface; workspace shell has independent scroll domains for center/side panels and hydration-safe persisted chrome state; gallery uses rendered document preview cards; mobile tabs scroll with minimum widths; mobile/tablet chrome sweep landed — a persistent (non-scrolling) bottom action bar exposes Panel/Context/Search/Settings on phones, a fixed context trigger covers the md–lg band, the left/right drawers use the `Sheet` primitive (backdrop-tap/ESC/focus-trap), the settings modal stacks below md, and drawer Gallery/Assets/Admin now navigate like the icon rail; archived documents close their workspace tabs and select the nearest remaining tab; tab reordering now works on touch (pointer-events with press-and-hold, replacing HTML5 drag); a final route consistency pass remains |
...
| [~] | Add folder-inherited default tags | `folders.default_tags` (migration `0022`, applied locally) hands tags to every document in a folder and its subfolders. Resolved at sync time by `lib/folder-tags.ts` and unioned into `document_tags` — never written into a document's frontmatter — with the editor's Properties panel showing them as locked chips. Owner-only to set, via the Folder settings dialog. Resolution, union, unfiling, soft-deleted ancestors, cycles, and subtree/delete-path ordering are all verified against real Postgres. `[~]` because the four server actions that wrap the re-sync helpers, and the dialog/Properties UI, have not been exercised end to end in a running app |
...
| [x] | Slice 2 — Hover card on read surfaces | Done 2026-09-17. `components/markdown/DefinitionPreviewCard.tsx` on `@base-ui/react/preview-card` (hover delay 250ms over the 600ms default, `safePolygon` path into the card, focus/Escape/outside-press for free). Reached from `MarkdownDocument`'s existing `a` override via `buildDefinitionsByHref` — a reverse `href -> definition` index cached against the resolution map's identity, so **no raw HTML and no sanitizer change**. The card takes the rendered preview as a **node**, not Markdown, so it never imports `MarkdownDocument` back (that cycle would resolve to `undefined` at module init); the caller renders it with `disableLinks`, which is also what caps preview depth at zero. CSS: `.vault-md-definition-link` is weight-only (550, no underline, no colour), card classes in `app/styles/components.css`, all four registered in `docs/CSS_CONTRACT.md`. Native `overflow-y` on the card body rather than Base UI `ScrollArea` — one less component tree for a scrollbar we do not restyle. 421 tests (+6) |
...
Slices 1-5 are implemented. All six languages run end to end locally under `runc`; slices 4 and 5 still need deploying to the mini-PC and verifying under gVisor.
...
| [x] | Slice 3 - Runner proof and backend selection | Done 2026-09-24 on the mini-PC (N150, 4 cores, gVisor `release-20260921.0`). **Isolation 12/12** — network/DNS denied, read-only root, unprivileged, no-new-privs, fork bomb and memory hog killed, timeout kill, gVisor kernel confirmed, mid-run cancellation, no surviving containers. **Six languages run**, warm: Python 254ms, JavaScript 302ms, C 344ms, C++ 829ms, Haskell 1382ms, Java 2747ms. Images 6.48 GB against 312 GB free. **C# dropped** — the only language needing a project scaffold rather than a loose file; it failed three runs for three unrelated scaffold reasons while the rest worked. Highlighting for C# is unaffected. Full numbers and conclusions in plan §6.1. Host/backend decisions: gVisor directly on the host, OCI only. Contention with the Minecraft server is untested — numbers assume it stopped |
| [x] | Slice 4 - Durable jobs and first execution integration | Done 2026-09-25, **locally** — not yet deployed or run under real gVisor. `code_jobs` queue (migration `0023`) with leases, attempt tokens and advisory-locked admission/claiming; `/api/code/*` user routes (cookie, cross-site guarded, allowlist + `canEdit`) and worker routes (bearer token only, never a cookie); host-side `runner/worker.mjs` passing argv straight to Docker with no shell; Run/Stop panel in the editor. Exit checks: Python and JS end to end, restart (hard-killed worker mid-job — sandbox stopped itself via the in-container backstop, job became `infrastructure_error`, restart reaped it without re-running), deduplication, revocation (ban takes effect next request), timeout, result expiry and cleanup all pass. Native formatters and a stdin field moved to slice 5: the API and runner support both, the editor does not surface them yet |
| [x] | Slice 5 - Complete compiled-language support | Done 2026-09-25, **locally** — not yet deployed or run under gVisor. Profiles `java-21`, `ghc-9.6`, `gcc-14-c`, `gcc-14-cpp` in `server/code-runtime.ts` with warnings on and pinned formatter argv; Format routes Ruff/google-java-format/Ormolu/clang-format languages to the runner; an **Input** toggle gives a block private stdin. 53 end-to-end checks against a real worker (51 first pass, 2 cold-cache timing failures passed 3/3 on retry; Java run limit then raised to 10s and not re-run). **Still open**: a browser pass of the Input box and runner Format, and `npm run build` — both skipped on the laptop for disk/RAM, to be done on the mini-PC. C# is out of scope as of 2026-09-24 |
...
| 2026-09-09 | The calc operand menu filters by grammar position, not by typing everything into one list | Offering all 160-odd ISO codes beside every bound name at every cursor is noise, and most of those options would not parse where they sat. The grammar already disambiguates — uppercase is money, lowercase is an identifier — so after `in`/`to` or a bare amount the menu shows currencies only, and elsewhere shows names + functions with currencies added once the token turns uppercase. Don't "simplify" this back to one undifferentiated list |
| 2026-09-09 | The `:::` menu is the slash items re-reached by a different trigger, not a second registry | `:::cal` and `/calcblock` run the *same* item and therefore insert identical markdown; a parallel list of directive blocks would drift the first time one side gained an option. Membership is a declared `directive?: string`, not sniffed from `insert.markdown`, because a contribution's markdown may be a factory that mints a fresh calendar id when called |
