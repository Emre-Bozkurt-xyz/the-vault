...
import { executionEnabled, listRuntimeProfiles, userMayExecute } from "@/server/code-runtime";
 ...
 * name, digest or argv. When execution is off or the user is not allowlisted
 * the answer is simply "nothing", which is also what an anonymous caller gets.
 */
export async function GET() {
  const user = await currentCodeUser();
  const enabled = executionEnabled() && !!user && userMayExecute(user.id);
